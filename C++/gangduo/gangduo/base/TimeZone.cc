...
const int nSecondsPerDay = 24*60*60;
    ...
    if(f.valid()){
        try{
            ...
            for(int i = 0; i < timecnt; ++i){
                ...
                trans.push_back(local);
            }

            for(int i = 0; i < timecnt; ++i){
                int32_t gmtoff = f.readInt32();
                uint8_t isdst = f.readUInt8();
                int8_t abbrind = f.readUInt8();
...
struct tm TimeZone::toUtcTime(time_t secondsSinceEpoch, bool yday){
    ...
    utc.tm_zone = "GMT";
    int seconds = static_cast<int>(secondsSinceEpoch % nSecondsPerDay);
    int days = static_cast<int>(secondsSinceEpoch / nSecondsPerDay);
    if(seconds < 0){
        seconds += nSecondsPerDay;
        --days;
    }
    detail::fillHMS(seconds, &utc);
    Date date(days + Date::nJulianDayOf1970_01_01);
...
time_t TimeZone::fromUtcTime(int year, int month, int day, int hour, int minute, int seconds){
    ...
    time_t days = date.julianDayNumber() - Date::nJulianDayOf1970_01_01;
    return days * nSecondsPerDay + secondsInDay;
}
